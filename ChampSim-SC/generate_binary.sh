...
# DO NOT REMOVE THE " "
# Auto-detect the ChampSim-SC root from this script's own location, so the
# package works from any directory without hand-editing paths.
CHAMPSIM_DIR="$(cd "$(dirname "$0")" && pwd)"

PATH_CACHE_H="${CHAMPSIM_DIR}/inc"

PATH_MAIN_CC="${CHAMPSIM_DIR}/src"
...
# Morrigan
if [ ${STLB_PREF} = "morriganPT" ] || [ ${STLB_PREF} = "morriganPT_tage" ] || [ ${STLB_PREF} = "morriganPT_tage2" ]; then
...
# RESET FREQUENCY
# Always rewritten. The Morrigan RLFU decay path reads RESET_FREQ too, but the
# original script only rewrote it for markov_sota, so a Morrigan build could
# silently inherit the value left behind by a previous markov_sota build.
sed -i '/#define RESET_FREQ/c\#define RESET_FREQ '${RESET_FREQ}'' ${PATH_CACHE_H}/cache.h

# REPLACEMENT POLICY
if [ ${STLB_PREF} = "morriganPT" ] || [ ${STLB_PREF} = "morriganPT_tage" ] || [ ${STLB_PREF} = "morriganPT_tage2" ] || [ ${STLB_PREF} = "markov_sota" ]; then
